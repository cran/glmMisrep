NormRegMisrepEM<-function(formula, v_star, data, lambda = c(0.6,0.4), epsilon = 1e-08, maxit = 10000, maxrestarts = 20, verb = FALSE) {
  ...
  obs.ll <- function(lambda, coef){
    sum(   v_star *log(            dnorm(x = y, sd = coef[1], mean =  x %*% coef[-1]  )))+
      sum((1-v_star)*log(lambda[2]*dnorm(x = y, sd = coef[1], mean =  X %*% coef[-1]  )+
                           lambda[1]*dnorm(x = y, sd = coef[1], mean =  x %*% coef[-1] )))
  }


  # M step loglikelihood
  mstep.ll <- function(theta, z){
    -sum(                  log(dnorm(x = y[v_star==1], sd = theta[1], mean = (as.vector( x %*% theta[-1] )[v_star==1]  ) )))-
      sum((1-z[v_star==0])*log(dnorm(x = y[v_star==0], sd = theta[1], mean = (as.vector( X %*% theta[-1] )[v_star==0]  ) ))+
            z[v_star==0] *log(dnorm(x = y[v_star==0], sd = theta[1], mean = (as.vector( x %*% theta[-1] )[v_star==0]  ) )))
  }
  ...
  while(diff > epsilon && iter < maxit){
    ...
    # E-step
    dens1 <- lambda[1]*dnorm(x = y, sd = theta[1], mean = ( xbeta))
    dens2 <- lambda[2]*dnorm(x = y, sd = theta[1], mean = ( X %*% theta[-1]))
    z <- dens1/(dens1+dens2)
  ...
  # Element 1,1
  hess[1,1] <- -sum((1 - v_star) * ( ( exp( -(y - X %*% theta[-1] )^2 / (2*sigma^2) ) - exp( -(y - x%*%theta[-1])^2/(2*sigma^2) ) )/ ( lambda[2]*exp( -(y - X %*% theta[-1] )^2 / (2*sigma^2) ) + lambda[1]*exp( -(y - x%*%theta[-1])^2/(2*sigma^2) ) ) )^2 )

  # Element 2,2
  hess[2,2] <- sum( v_star*(1/sigma^2 - 3*(y - x%*%theta[-1])^2/sigma^4) + (1-v_star) * ( 1/sigma^2 - ( 3*(y-X%*%theta[-1])^2/sigma^4 * lambda[2]^2*exp( -(y-X%*%theta[-1])^2/(2*sigma^2) )^2 - lambda[2]*lambda[1]*exp( - ((y-X%*%theta[-1])^2 + (y-x%*%theta[-1])^2)/(2*sigma^2) ) * ( ( ((y-x%*%theta[-1])^2 - (y-X%*%theta[-1])^2)/sigma^3 )^2 - ( 3*(y-x%*%theta[-1])^2 + 3*(y-X%*%theta[-1])^2 )/sigma^4 ) + 3*(y-x%*%theta[-1])^2/sigma^4*lambda[1]^2*exp(-(y-x%*%theta[-1])^2/(2*sigma^2))^2 ) / ( lambda[2]*exp(-(y-X%*%theta[-1])^2/(2*sigma^2)) + lambda[1]*exp(-(y-x%*%theta[-1])^2/(2*sigma^2)) )^2 ) )

  # Main diagonal elements that pertain to regression coefficients;
  for(j in 1:ncol(x)){
    k <- j
    hess[j+2,k+2] <- sum(-v_star*(x[,k]*x[,j])/sigma^2 - (1-v_star) * (lambda[2]^2 * exp(-(y-X%*%theta[-1])^2/(2*sigma^2))^2 * (X[,k]*X[,j])/sigma^2 - lambda[2]*lambda[1]*exp(-( (y-X%*%theta[-1])^2 + (y-x%*%theta[-1])^2  )/(2*sigma^2) ) * ( ( (x[,k]*(y-x%*%theta[-1])-X[,k]*(y-X%*%theta[-1]))/sigma^2 ) * ( (x[,j]*(y-x%*%theta[-1]) - X[,j]*(y-X%*%theta[-1]))/sigma^2 ) - (x[,k]*x[,j]+X[,j]*X[,k])/sigma^2 ) + lambda[1]^2*exp(-(y-x%*%theta[-1])^2/(2*sigma^2))^2 *(x[,k]*x[,j])/sigma^2 ) / (lambda[2]* exp(-(y-X%*%theta[-1])^2/(2*sigma^2)) + lambda[1]*exp(-(y-x%*%theta[-1])^2/(2*sigma^2)) )^2 )
  }

  # Elements 2,1, 1,2
  hess[2,1] <- sum((1-v_star) * ( exp( -( (y-X%*%theta[-1])^2 + (y-x%*%theta[-1])^2 )/(2*sigma^2) ) * ( (y-X%*%theta[-1])^2 - (y-x%*%theta[-1])^2 )/sigma^3 ) / ( lambda[2]*exp(-(y-X%*%theta[-1])^2/(2*sigma^2)) + lambda[1]*exp(-(y-x%*%theta[-1])^2/(2*sigma^2)) )^2 )
  hess[1,2] <- sum((1-v_star) * ( exp( -( (y-X%*%theta[-1])^2 + (y-x%*%theta[-1])^2 )/(2*sigma^2) ) * ( (y-X%*%theta[-1])^2 - (y-x%*%theta[-1])^2 )/sigma^3 ) / ( lambda[2]*exp(-(y-X%*%theta[-1])^2/(2*sigma^2)) + lambda[1]*exp(-(y-x%*%theta[-1])^2/(2*sigma^2)) )^2 )

  # Off-diagonal elements-- lambda, regression coefs
  for(j in 1:ncol(x)){
    hess[j+2,1] <- sum( (1-v_star)* ( exp(-( (y-X%*%theta[-1])^2 + (y-x%*%theta[-1])^2 )/(2*sigma^2) ) * ( ( X[,j]*(y-X%*%theta[-1]) - x[,j]*(y-x%*%theta[-1]) )/sigma^2 ) ) / ( lambda[2]*exp(-(y-X%*%theta[-1])^2/(2*sigma^2)) + lambda[1]*exp(-(y-x%*%theta[-1])^2/(2*sigma^2)) )^2 )
    hess[1,j+2] <- sum( (1-v_star)* ( exp(-( (y-X%*%theta[-1])^2 + (y-x%*%theta[-1])^2 )/(2*sigma^2) ) * ( ( X[,j]*(y-X%*%theta[-1]) - x[,j]*(y-x%*%theta[-1]) )/sigma^2 ) ) / ( lambda[2]*exp(-(y-X%*%theta[-1])^2/(2*sigma^2)) + lambda[1]*exp(-(y-x%*%theta[-1])^2/(2*sigma^2)) )^2 )
  }

  # Off-diagonal elements-- sigma, regression coefs
  for(k in 1:ncol(x)){
    hess[k+2,2] <- sum(-v_star * (2*x[,k]*(y-x%*%theta[-1]))/sigma^3 - (1-v_star)* ( lambda[2]^2*exp(-(y-X%*%theta[-1])^2/(2*sigma^2))^2*(2*X[,k]*(y-X%*%theta[-1]))/sigma^3 - lambda[2]*lambda[1]*exp(- ( (y-X%*%theta[-1])^2 + (y-x%*%theta[-1])^2 )/(2*sigma^2) ) * ( ( (x[,k]*(y-x%*%theta[-1]) -X[,k]*(y-X%*%theta[-1]) )/sigma^2 ) * (( (y-x%*%theta[-1])^2 - (y-X%*%theta[-1])^2 )/sigma^3) - ( 2*x[,k]*(y-x%*%theta[-1])+2*X[,k]*(y-X%*%theta[-1]) )/sigma^3 ) + lambda[1]^2*exp(-(y-x%*%theta[-1])^2/(2*sigma^2))^2 *(2*x[,k]*(y-x%*%theta[-1]))/sigma^3  ) / ( lambda[2]*exp(-(y-X%*%theta[-1])^2/(2*sigma^2)) + lambda[1]*exp(-(y-x%*%theta[-1])^2/(2*sigma^2)) )^2 )
    hess[2,k+2] <- sum(-v_star * (2*x[,k]*(y-x%*%theta[-1]))/sigma^3 - (1-v_star)* ( lambda[2]^2*exp(-(y-X%*%theta[-1])^2/(2*sigma^2))^2*(2*X[,k]*(y-X%*%theta[-1]))/sigma^3 - lambda[2]*lambda[1]*exp(- ( (y-X%*%theta[-1])^2 + (y-x%*%theta[-1])^2 )/(2*sigma^2) ) * ( ( (x[,k]*(y-x%*%theta[-1]) -X[,k]*(y-X%*%theta[-1]) )/sigma^2 ) * (( (y-x%*%theta[-1])^2 - (y-X%*%theta[-1])^2 )/sigma^3) - ( 2*x[,k]*(y-x%*%theta[-1])+2*X[,k]*(y-X%*%theta[-1]) )/sigma^3 ) + lambda[1]^2*exp(-(y-x%*%theta[-1])^2/(2*sigma^2))^2 *(2*x[,k]*(y-x%*%theta[-1]))/sigma^3  ) / ( lambda[2]*exp(-(y-X%*%theta[-1])^2/(2*sigma^2)) + lambda[1]*exp(-(y-x%*%theta[-1])^2/(2*sigma^2)) )^2 )
  }

  # Off-diagonal elements-- regression coefficients
  for(i in 1:choose(ncol(x),2)){
    j <- combn(x = 1:ncol(x), m = 2)[1,i]
    k <- combn(x = 1:ncol(x), m = 2)[2,i]

    hess[k + 2, j + 2] <- sum(-v_star*(x[,k]*x[,j])/sigma^2 - (1-v_star) * (lambda[2]^2 * exp(-(y-X%*%theta[-1])^2/(2*sigma^2))^2 * (X[,k]*X[,j])/sigma^2 - lambda[2]*lambda[1]*exp(-( (y-X%*%theta[-1])^2 + (y-x%*%theta[-1])^2  )/(2*sigma^2) ) * ( ( (x[,k]*(y-x%*%theta[-1])-X[,k]*(y-X%*%theta[-1]))/sigma^2 ) * ( (x[,j]*(y-x%*%theta[-1]) - X[,j]*(y-X%*%theta[-1]))/sigma^2 ) - (x[,k]*x[,j]+X[,j]*X[,k])/sigma^2 ) + lambda[1]^2*exp(-(y-x%*%theta[-1])^2/(2*sigma^2))^2 *(x[,k]*x[,j])/sigma^2 ) / (lambda[2]* exp(-(y-X%*%theta[-1])^2/(2*sigma^2)) + lambda[1]*exp(-(y-x%*%theta[-1])^2/(2*sigma^2)) )^2 )
    hess[j + 2, k + 2] <- sum(-v_star*(x[,k]*x[,j])/sigma^2 - (1-v_star) * (lambda[2]^2 * exp(-(y-X%*%theta[-1])^2/(2*sigma^2))^2 * (X[,k]*X[,j])/sigma^2 - lambda[2]*lambda[1]*exp(-( (y-X%*%theta[-1])^2 + (y-x%*%theta[-1])^2  )/(2*sigma^2) ) * ( ( (x[,k]*(y-x%*%theta[-1])-X[,k]*(y-X%*%theta[-1]))/sigma^2 ) * ( (x[,j]*(y-x%*%theta[-1]) - X[,j]*(y-X%*%theta[-1]))/sigma^2 ) - (x[,k]*x[,j]+X[,j]*X[,k])/sigma^2 ) + lambda[1]^2*exp(-(y-x%*%theta[-1])^2/(2*sigma^2))^2 *(x[,k]*x[,j])/sigma^2 ) / (lambda[2]* exp(-(y-X%*%theta[-1])^2/(2*sigma^2)) + lambda[1]*exp(-(y-x%*%theta[-1])^2/(2*sigma^2)) )^2 )
  ...
  a=list(y = y, lambda = lambda[2], params = theta, loglik = new.obs.ll,
         ...
         ICs = perf_metrics, ft="NormRegMisrepEM", formula = formula, v_star_name = v_star_name)
  class(a) = "misrepEM"
